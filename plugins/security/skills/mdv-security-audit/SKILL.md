...
Write `recon.md` (~15 lines, human-readable) and `recon.json` (structured — see `references/recon-schema.md`) into `.claude/security/`. Create the directory if missing. If `.claude/security/` is not writable, fall back to `<repo>/mdv-security-audit/` or echo to chat.
