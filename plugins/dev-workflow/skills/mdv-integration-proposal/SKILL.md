...
Not for: implementing the feature (use `/mdv-code-implement-feature`), pure cost numbers (use `/mdv-task-estimate`), or codebase convention docs (use `/mdv-code-analyze-codebase`).
...
| Odhad pracnosti — delegate to `/mdv-task-estimate` | ◻ | ◻ (only as price, never sizing) |
