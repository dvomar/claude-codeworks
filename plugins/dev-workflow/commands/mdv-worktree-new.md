...
1. **Idempotency**: read `${MAIN_REPO}/.claude/worktrees.json` if it exists. If an entry has matching `task_id` and `status == "active"`, print its existing path and exit 0:
   ...
   Run /mdv-worktree-list to see all worktrees.
   ```
...
|---|---|---|
| `/mdv-worktree-new CF-42 fix payment flow` | `task/CF-42-fix-payment-flow` | `../<repo>-fix-payment-flow` |
| `/mdv-worktree-new TUKAS-7 add CSV export` | `task/TUKAS-7-add-csv-export` | `../<repo>-add-csv-export` |
| `/mdv-worktree-new #123 quick patch --offline` | `task/123-quick-patch` | `../<repo>-quick-patch` |
| `/mdv-worktree-new CF-42 refactor auth --base develop` | `task/CF-42-refactor-auth` (from `origin/develop`) | `../<repo>-refactor-auth` |
