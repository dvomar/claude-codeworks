---
name: mdv-code-design-ui-ux
description: 'Design the UI/UX for code that has no usable interface yet — a backend with no frontend, an internal tool nobody can use, a CLI that needs a GUI, an API that needs a console. Reads the codebase, infers what the product is for and who uses it, then delivers information architecture, screen inventory, interaction and state model, design tokens and accessibility as a written spec plus a clickable self-contained HTML prototype. Every screen is grounded in real entities, statuses, roles and error paths found in the code. Use for "navrhni UI/UX", "jak by to mělo vypadat", "udělej z toho použitelnou appku", "design a frontend for this service", "what should the interface look like", "propose a UX for this module". Pick by input: code with no usable UI yet → this skill; an existing UI to improve → /mdv-optimize-ui-ux; a brief with no codebase behind it → /mdv-frontend-design.'
...
`/mdv-frontend-design`), reviewing frontend code quality (`/mdv-code-review-feature`),
scoping and pricing the build (`/mdv-task-estimate`), or a stakeholder approval document
about an integration (`/mdv-integration-proposal`).

## Usage

```
/mdv-code-design-ui-ux [target] [--key value]
```
...
You are not mapping the architecture (that is `/mdv-code-analyze-codebase`). You are
...
Offer `/mdv-html-to-pdf` for a shareable PDF of the spec.
...
`/mdv-code-implement-feature`, so the people *building* the next module inherit it too,
...
Related: `/mdv-code-analyze-codebase` (run first on an unfamiliar repo — its
`knowledge/` output makes Phase 1 much faster) · `/mdv-frontend-design` (implement the
result with a strong visual point of view) · `/mdv-code-implement-feature` (build it —
it picks up `design-language.md` from `knowledge/` automatically) · `/mdv-html-to-pdf`
(share the spec).
