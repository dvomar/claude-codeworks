---
name: optimize-ui-ux
description: Optimize the UI/UX of a specific element, screen, flow, or a whole block of an app — analyze the current code, research current best practices (context7 + web), then produce 3 genuinely different self-contained HTML proposals each covering desktop, tablet and mobile, compare them, recommend one, and implement the chosen one. Use this whenever the user asks to improve, redesign, rethink or "optimize" any piece of interface, navigation, form, header, dashboard or flow — including phrasings like "optimalizuj UI/UX", "předělej design", "navrhni jak vylepšit", "vytvoř návrhy", "3 návrhy", "udělej to hezčí/přehlednější", "improve the UX of X", "redesign the login/nav/dashboard". Trigger it even when the user only describes the annoyance ("tohle je nepřehledné", "nikdo to nenajde", "zalamuje se to") without using the word design.
user-invocable: true
allowed-tools: Read, Write, Edit, Grep, Glob, Bash, WebSearch, WebFetch, AskUserQuestion, mcp__context7__resolve-library-id, mcp__context7__query-docs
---
